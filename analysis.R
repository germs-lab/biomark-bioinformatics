...
for (x in 1:length(gene_list)) {
   ...
   f$log <- log10(f$rConc)
   p = ggplot(f, aes_string(x="log", y="MEAN", color="Plate"))
...
# Here is where you define the index of the gene you are looking for the standards for x = index in gene_list
x = 7
 {
  each_gene <- subset(standards, Name == gene_list[x])
  standard_only = paste(gene_list[x], '-', gene_list[x], sep='')
...
f <- ddply(water, .(full_assay, plot, treatment, Name, Name.1), summarise, MEAN = mean(Value), SE=sd(Value)/sqrt(length(Value)))
...
f_plot <-ddply(f, .(treatment, Name.1), summarise, MEAN2 = mean(MEAN), SE2=sd(MEAN)/sqrt(length(MEAN)))
...
p + geom_point() + geom_errorbar(width=0.25, limits2)+theme_classic()+ theme(text=element_text(size=15, family="Helvetica"),axis.text.x = element_text(angle = 90), legend.position="none") + facet_grid(~treatment)
